...
const searchMovies = async searchText => {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      updateOptions(xhttp.response);
    }
  };
  xhttp.open("GET", "query/" + searchText, true);
  xhttp.send();
}
...
function makeArray(response_str) {
  // Makes an array from the response text.
  response_str = response_str.slice(1, -1);

  let arr = [];
  let start = true;
  let option = '';
  for (let i = 0; i < response_str.length; i++) {
    let char = response_str[i];
    if (char === '"' && start){
      start = false;
      continue;
    } else if (char === ','&& !start) {
